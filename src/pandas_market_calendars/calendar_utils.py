...
def date_range(schedule, frequency, closed='right', force_close=True, timezone="UTC", **kwargs):
    """
    ...
    for row in schedule.itertuples():
        dates = pd.date_range(row.market_open, row.market_close, freq=frequency, tz=timezone, **kwargs)
        if force_close:
