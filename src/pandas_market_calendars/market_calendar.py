...
class MarketCalendar(metaclass=MarketCalendarMeta):
    ...
    def open_at_time(schedule, timestamp, include_close=False):
        ...
        Steps:
            1) Get the current and next day's date (needed for securities that close and re-open on the same calendar day).
            2) Check to see if market is open on that date, if not, go to step 6.
            3) Check to see if timestamp >= currentDayMarketOpen, if not, return False
            4) If there's a break:
                4a) Check to see if timestamp < (<= if include_close) breakStart, if so, return True.
                4b) If it isn't, check to see if timestamp >= to breakEnd and < (<=  if include_close) currentDayMarketClose, if so, return True.
            5) If there's no break, check to see if timestamp < (<= if include_close) currentDayMarketClose, if so, return True.
            6) Check to see if market is open on the following day, if so, check to see if nextDayMarketOpen <= timestamp <  the following days date at midnight, with the same timezone as timestamp. If so, return True.
            7) return False.
        ...
        """
        # Note: The schedule is in UTC, so if removing timezone to get date, we need to convert timestamp to UTC first
        timestampUTC = timestamp.astimezone(tz=tz.tzutc())
        currentDateAtZeroHundredHoursNoTz = timestampUTC.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=None)
        nextDateAtZeroHundredHoursNoTz = currentDateAtZeroHundredHoursNoTz + timedelta(days=1)
        # This is important because we can't compare the timestamp to the next date at midnight unless they both have the same timezone.
        nextDateAtZeroHundredHoursWithTz = timestampUTC.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)

        if currentDateAtZeroHundredHoursNoTz in schedule.index: # See if market is open on current date
            marketOpen = schedule.loc[currentDateAtZeroHundredHoursNoTz, 'market_open']
            marketClose = schedule.loc[currentDateAtZeroHundredHoursNoTz, 'market_close']
            if marketOpen <= timestamp and timestamp <= marketClose:
                if 'break_start' in schedule.columns:
                    breakStart = schedule.at[currentDateAtZeroHundredHoursNoTz, 'break_start']
                    breakEnd = schedule.at[currentDateAtZeroHundredHoursNoTz, 'break_end']
                    if include_close:
                        if timestamp <= breakStart:
                            return True
                        elif breakEnd <= timestamp:
                            return True
                    else:
                        if timestamp < breakStart:
                            return True
                        elif breakEnd <= timestamp and timestamp < marketClose:
                            return True
                elif include_close: # Already verified the other required conditions for this block in previous 'if' statement.
                    return True
                elif timestamp < marketClose:
                    return True
        # This can't be an elif statement because, e.g., at 20:00 on a normal Tuesday, while the CME was open on that Tuesday's date, it is open (has reopened), but its 'actual' open date is the following Wednesday's date. So, we need to check two dates.
        if nextDateAtZeroHundredHoursNoTz in schedule.index: #This is for cases such as those with futures; the actual open day in the schedule is the following day, but marketOpen should still fall on the requested day.
            nextDayMarketOpen = schedule.loc[nextDateAtZeroHundredHoursNoTz, 'market_open']
            if include_close and nextDayMarketOpen <= timestamp and timestamp <= nextDateAtZeroHundredHoursWithTz:
                return True
            elif nextDayMarketOpen <= timestamp and timestamp < nextDateAtZeroHundredHoursWithTz:
                return True
        return False
